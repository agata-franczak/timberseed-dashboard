...
- Live data: meetings fetched from RecruitCRM API (daily)
- Static data: dist job counts + pipeline read from static_data.json (update weekly via Claude)
"""
...
def fetch_meetings(owner_id):
    ...
    while True:
        ...
        batch = body.get("data", [])
        print(f"  meetings {owner_id} p{page}: {len(batch)}", flush=True)
        if not batch:
            break
        in_range = [m for m in batch if (m.get("start_date") or "")[:10] >= cutoff]
        out.extend(in_range)
        oldest = min((m.get("start_date") or "")[:10] for m in batch)
        if oldest < cutoff or not body.get("next_page_url") or page >= 20:
            break
        page += 1
    return out
...
function render(s,e){{
  ...
  document.getElementById("hdates").innerHTML=
    `Calls updated: ${{fGB(g)}} ${{g.toLocaleTimeString("en-GB",{{hour:"2-digit",minute:"2-digit"}})}}<br>`+
    (su?`Dist & pipeline updated: ${{fGB(su)}}`:"");
...
function bC(c,s,e){{
  ...
  const funnel=SO.map(st=>{{
    ...
    const nm=ca.slice(0,4).map(p=>{{const pts=p.name.split(" ");const sh=pts[0]+(pts[1]?" "+pts[1][0]+".":"");
  ...
  const body=isFinn?
    ...
    `<div><div class="secl">Owned candidates at key stages</div>
      ${{pipe===0?`<div class="nodata">No owned candidates at key stages in this period.</div>`:`<div class="fn">${{funnel}}</div>`}}</div>
    <div class="note">Dist. job and pipeline data updated manually \u2014 see header for date.</div>`;
...
if __name__ == "__main__":
    ...
    if static:
        ...
        print("WARNING: static_data.json not found — dist job and pipeline will be empty")
